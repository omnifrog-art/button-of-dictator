...
def supa_active_subs() -> set:
    url = f"{SUPA_URL}/rest/v1/logs?select=subdomain,status&limit=1000"
    resp = requests.get(url, headers=s_headers(), timeout=15)
    resp.raise_for_status()
    data = resp.json()
    return {r["subdomain"] for r in data if r.get("status") != "terminated"}

def supa_all_subs() -> set:
    url = f"{SUPA_URL}/rest/v1/logs?select=subdomain&limit=1000"
    resp = requests.get(url, headers=s_headers(), timeout=15)
    resp.raise_for_status()
    data = resp.json()
...
def main():
    ...
    while added < need_count and attempts < 300:
        attempts += 1
        # 动态更新，防止添加到一半 live已满
        active_subs = supa_active_subs()
        live_count = len(active_subs)
        ...
        if need_count <= 0:
            print("✓ Pool dynamically filled, exiting early.")
            break
    ...
    print(f"✓ Added {added} new sub-domains, finished.")
    sys.exit(0)
